...
inline void SolveFromCSV(const string& inputCsvPath, const string& outputCsvPath, const string& mode) {
    ...
    while (getline(inFile, line)) {
        ...
        cout << "已處理盤面: " << boardStr << " | 步數: " << actionNum << "\n";
    }

    inFile.close();
    outFile.close();
    cout << "所有題目解題完畢，解答已存至: " << outputCsvPath << endl;
}
